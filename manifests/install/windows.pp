...
#
# Download and install winlogbeat on Windows
#
# @summary A private class that installs winlogbeat on Windows
#
class winlogbeat::install::windows {
  ...
  {
    $unzip_command = "Expand-Archive ${zip_file} \"${winlogbeat::install_dir}\""
  }
  else
  {
    $unzip_command = "\$sh=New-Object -COM Shell.Application;\$sh.namespace((Convert-Path '${winlogbeat::install_dir}')).Copyhere(\$sh.namespace((Convert-Path '${zip_file}')).items(), 16)" # lint:ignore:140chars
  }
